-- ================================================
-- doodleduo Supabase Hotfix Bundle (Run once)
-- Includes:
--  1. Adds room_name column to duo_rooms
--  2. Rebuilds notify_partner_of_activity trigger function (SECURITY DEFINER + activity metadata)
--  3. Removes legacy trigger & recreates notify_partner_on_activity
-- ================================================

BEGIN;

-- 1) Add friendly room names (safe if column already exists)
ALTER TABLE public.duo_rooms
ADD COLUMN IF NOT EXISTS room_name text;

COMMENT ON COLUMN public.duo_rooms.room_name IS
    'Optional display name chosen by the couple (ex. Cozy Farm)';

-- 2) Recreate trigger function with full payload + RLS bypass
...
COMMENT ON FUNCTION public.notify_partner_of_activity() IS
    'Sends push notifications when new partner activities are created (security definer, includes activity_id + widget refresh flag)';

-- 3) Clean up triggers so only one fires
...
DO $$
BEGIN
    RAISE NOTICE '✅ Room name column ensured';
    RAISE NOTICE '✅ notify_partner_of_activity recreated as SECURITY DEFINER';
    RAISE NOTICE '✅ Legacy trigger removed and new trigger attached';
    RAISE NOTICE '🎉 Done! Test by sending an activity with both apps closed.';
END $$;
