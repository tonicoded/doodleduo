-- Enrich push payload with activity IDs so clients can fetch doodles in the background
-- Also flag doodle pushes for widget refreshes.
...
COMMENT ON FUNCTION public.notify_partner_of_activity() IS 'Sends push notifications when new partner activities are created (includes activity_id and widget_refresh flag)';
