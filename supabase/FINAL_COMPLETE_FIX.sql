-- ================================================
-- FINAL COMPLETE FIX - Restore Everything
-- This restores the exact working state from migration 011
-- ================================================

BEGIN;

-- 1. Drop any existing broken triggers
...
-- 2. Recreate the EXACT working trigger function from migration 011
...
BEGIN
    ...
    IF room_member_count = 2 THEN
        -- Get partner's user ID and display name (THIS IS THE KEY FIX)
...
-- 4. Verify everything
SELECT '✅ Trigger restored successfully!' as status;

SELECT
    'Trigger exists: ' || CASE WHEN EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'notify_partner_on_activity'
...
SELECT
    'Function exists: ' || CASE WHEN EXISTS (
        ...
        WHERE n.nspname = 'public' AND p.proname = 'notify_partner_of_activity'
